...
extension MerchantDelegate {
    /// Default trapping implementation of `MerchantDelegate.merchant(_:consume:completion:)`. This delegate method must be implemented by an application that allows users to purchase consumable products. It can be ignored if the application only handles non-consumable, or subscription products.
    public func merchant(_ merchant: Merchant, consume product: Product, completion: @escaping () -> Void) {
        MerchantKitFatalError.raise("Implement `MerchantDelegate.merchant(_:consume:completion)` to respond to purchases of consumable products. It is a programming error not to explicitly implement the delegate method if the application allows users to purchase consumable products.")
    }
