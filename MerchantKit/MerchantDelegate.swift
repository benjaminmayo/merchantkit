...
public protocol MerchantDelegate : class {
    ...
    /// A central place where errors are propagated when encountered by the `Merchant`. There is no expectation that these errors need to be handled but it may come in useful when logging/debugging. Use the `category` to refine your checks to a particular scope. A no-op default implementation of this method is provided.
