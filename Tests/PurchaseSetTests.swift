...
class PurchaseSetTests : XCTestCase {
    ...
    func testSinglePurchase() {
        let testPurchase = Purchase(from: self.mockedSource(forIdentifier: "test", price: "1.00"), characteristics: [])
    ...
    func testMultiplePurchases() {
        let testPurchase = Purchase(from: self.mockedSource(forIdentifier: "test", price: "1.00"), characteristics: [])
        let differentTestPurchase = Purchase(from: self.mockedSource(forIdentifier: "test2", price: "2.00"), characteristics: [])
    ...
    func testSortingPurchases() {
        let cheapestPurchase = Purchase(from: self.mockedSource(forIdentifier: "1", price: "1.00"), characteristics: [])
        let mediumPricedPurchase = Purchase(from: self.mockedSource(forIdentifier: "2", price: "2.00"), characteristics: [])
        let mostExpensivePurchase = Purchase(from: self.mockedSource(forIdentifier: "3", price: "3.00"), characteristics: [])
    ...
    private func mockedSource(forIdentifier productIdentifier: String, price: String) -> Purchase.Source {
        ...
        return .availableProduct(mockProduct)
    }
}
